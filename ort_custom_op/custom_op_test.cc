...
bool TestInference(Ort::Env &env, T model_uri,
    ...
    ort_outputs = session.Run(Ort::RunOptions{nullptr}, input_names.data(), input_tensors.data(), input_tensors.size(),
                              &output_name, 1);


    Ort::Value output_tensor{nullptr};
    output_tensor = Ort::Value::CreateTensor<float>(memory_info, const_cast<float *>(expected_values_y.data()),
                                                    expected_values_y.size(), expected_dims_y.data(),
...
int main(int argc, char **argv) {
    ...
    input->name = "X";
    input->dims = {3, 2, 1, 2};
    input->values = {1.5410f, -0.2934f, -2.1788f, 0.5684f, -1.0845f, -1.3986f, 0.4033f, 0.8380f, -0.7193f, -0.4033f,
                     -0.5966f, 0.1820f};
    ...
    std::vector<float> expected_values_y = {3.0000f, -1.0000f, -1.0000f, 1.0000f, 2.9996f, -0.9996f, -0.9999f, 0.9999f,
                                            -0.9996f, 2.9996f, -1.0000f, 1.0000f};
